...
func (c *Controller) processNextItem() bool {
	...
	// Handle the error if something went wrong during the execution of the bussiness logic
...
func (c *Controller) syncToStdout(key string) error {
	...
	if err != nil {
		klog.Errorf("Fetching object with key %s from stire failed with %v", key, err)
		return err
	}
	...
	if !exists {
		...
		// 这里就是真正的业务逻辑代码了，一般会比较spec和status的差异，然后做出处理使得status与spec保持一致，
		// 此处为了代码简单仅仅打印一行日志
...
func (c *Controller) handleErr(err error, key interface{}) {
	...
	if c.queue.NumRequeues(key) < 5 {
		klog.Infof("Error sycing pod %v: %v", key, err)
