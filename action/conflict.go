...
const (
	...
	// 用于更新的标签的名字
	LABEL_CUSTOMIZE string = "biz-verison"
)

type Conflict struct{}

func (conflict Conflict) DoAction(clientset *kubernetes.Clientset) error {
	...
	for i := 0; i < testNum; i++ {
		go func(clientsetA *kubernetes.Clientset, index int) {
			...
			// err := updateByGetAndUpdate(clientA, DP_NAME)
